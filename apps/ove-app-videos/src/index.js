      ...
      for (let i = 0; i < Constants.CLOCK_SYNC_ATTEMPTS; i++) {
        ...
        if (clock.syncResults.length + i < Constants.CLOCK_SYNC_ATTEMPTS) {
          setTimeout(
            clockSyncRequest,
            (Math.random() *
              (Constants.CLOCK_SYNC_INTERVAL / Constants.CLOCK_SYNC_ATTEMPTS)) |
              0
          );
        }
      }
  ...
  __self.sync = function (data) {
    if (data.sync) {
      if (!data.sync.t2) {
        ...
        let diff =
          ((syncResult.sync.t1 + syncResult.sync.t3) / 2 -
            syncResult.sync.t2 -
            syncResult.sync.serverDiff) |
          0;
        /* istanbul ignore if */
        ...
        log.trace(
          'Clock skew detection attempt:',
          clock.syncResults.length,
          'difference:',
          diff
        );
        if (clock.syncResults.length === Constants.CLOCK_SYNC_ATTEMPTS) {
    ...
    socket.on('message', function (msg) {
      ...
      if (m.appId === Constants.APP_NAME && m.message.bufferStatus) {
        ...
        if (status.type.registration) {
          ...
          if (
            status.percentage >= Constants.MIN_BUFFERED_PERCENTAGE ||
            status.duration >= Constants.MIN_BUFFERED_DURATION
          ) {
            bufferStatus.clients.splice(
              bufferStatus.clients.indexOf(status.clientId),
              1
            );
...
const handleOperation = function (req, res) {
  ...
  if (name === Constants.Operation.BUFFER_STATUS) {
    ...
    res
      ...
      .send(
        JSON.stringify({
          status: isComplete
            ? Constants.BufferStatus.COMPLETE
            : Constants.BufferStatus.BUFFERING,
        })
      );
    return;
  }
  ...
  if (name === Constants.Operation.SEEK) {
    ...
    message.operation.mute =
      typeof req.query.mute !== 'undefined' &&
    ...
    // be overridden.
    message.operation.loop =
      typeof req.query.loop !== 'undefined' &&
  ...
  res
    .status(HttpStatus.OK)
    .set(Constants.HTTP_HEADER_CONTENT_TYPE, Constants.HTTP_CONTENT_TYPE_JSON)
    .send(Utils.JSON.EMPTY);
};

let operationsList = Object.values(Constants.Operation);
operationsList.splice(
  operationsList.indexOf(Constants.Operation.BUFFER_STATUS)
);
...
app.get(
  '/operation/:name(' + Constants.Operation.BUFFER_STATUS + ')',
  handleOperation
);
