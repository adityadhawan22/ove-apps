...
if (process.env.OVE_MAPS_LAYERS) {
  log.info(
    'Loading map layers from environment variable:',
    process.env.OVE_MAPS_LAYERS
  );
...
base.operations.canTransform = function (state, transformation) {
  const combinations = [
    [
      ...
      'transformation.zoom',
      'transformation.pan',
      'transformation.pan.x',
      'transformation.pan.y',
      'state.center',
      'state.resolution',
    ],
    [
      ...
      'state.position.resolution',
      'state.position.bounds',
      'state.position.bounds.x',
      'state.position.bounds.y',
      'state.position.bounds.w',
  ...
  log.debug(
    'Can' + (canTransform ? '' : "'t") + 'transform state:',
    state,
    'using:',
    transformation
  );
...
base.operations.transform = function (input, transformation) {
  ...
  if (transformation.pan) {
    if (output.center) {
      ...
      output.center[0] =
        +output.center[0] + transformation.pan.x * output.resolution;
      output.center[1] =
        +output.center[1] - transformation.pan.y * output.resolution;
    } else {
      output.position.center[0] =
        +output.position.center[0] +
        transformation.pan.x * output.position.resolution;
      output.position.center[1] =
        +output.position.center[1] -
        transformation.pan.y * output.position.resolution;
      output.position.bounds.x =
        +output.position.bounds.x +
        transformation.pan.x * output.position.resolution;
      output.position.bounds.y =
        +output.position.bounds.y -
        transformation.pan.y * output.position.resolution;
    }
  }
  if (transformation.zoom) {
    if (output.resolution) {
      ...
      output.position.zoom =
        +output.position.zoom + Math.ceil(Math.log2(transformation.zoom));
      output.position.bounds.w /= transformation.zoom;
      output.position.bounds.h /= transformation.zoom;
      output.position.bounds.x =
        (output.position.center[0] * (transformation.zoom - 1) +
          output.position.bounds.x) /
        transformation.zoom;
      output.position.bounds.y =
        (output.position.center[1] * (transformation.zoom - 1) +
          output.position.bounds.y) /
        transformation.zoom;
    }
  }
  log.debug(
    'Successfully transformed state from:',
    input,
    'to:',
    output,
    'using:',
...
base.operations.canDiff = function (source, target) {
  const getCanDiff = function (state) {
    ...
    combinations.forEach(function (e) {
      ...
      e.forEach(function (x) {
        result =
          result &&
          !Utils.isNullOrEmpty(Utils.JSON.getDescendant(x, { state: state }));
      });
  ...
  log.debug(
    'Can' + (result ? '' : "'t") + 'difference source:',
    source,
    'and target:',
    target
  );
...
base.operations.diff = function (source, target) {
  ...
  log.debug(
    'Successfully computed difference:',
    result,
    'from source:',
    source,
    'to target:',
...
base.operations.validateState = function (state) {
  return (
    ...
    Utils.validateState(state, [
      { value: ['state.center', 'state.resolution', 'state.zoom'] },
    ]) ||
...
setTimeout(function () {
  const getSocket = function () {
    ...
    socket.on('close', function (code) {
      ...
      log.warn(
        'Attempting to reconnect in ' + Constants.SOCKET_REFRESH_DELAY + 'ms'
      );
...
const handleOperation = function (req, res) {
  ...
  if (sectionId) {
    ...
    ws.safeSend(
      JSON.stringify({ appId: Constants.APP_NAME, message: message })
    );
  }

  res
    .status(HttpStatus.OK)
    .set(Constants.HTTP_HEADER_CONTENT_TYPE, Constants.HTTP_CONTENT_TYPE_JSON)
    .send(JSON.stringify({}));
};
