...
app.use(
  '/',
  express.static(path.join(nodeModules, 'openvidu-browser', 'static', 'js'))
);
...
if (!Constants.OPENVIDU_HOST) {
  ...
  const ov = new OpenVidu(
    Constants.OPENVIDU_HOST,
    process.env.OPENVIDU_SECRET || config.secret
  );
  app.get('/operation/token', function (req, res) {
    ...
    if (session) {
      ...
      ov.createSession({ customSessionId: req.query.sessionId })
        .then((session) => {
          session
            .generateToken({ role: OpenViduRole.SUBSCRIBER })
            .then((token) => {
