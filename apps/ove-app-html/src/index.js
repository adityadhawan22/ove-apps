...
app.use('/libs/:name(distributed).js', function (req, res) {
  request(
    'http://' + Utils.getOVEHost() + '/jquery.min.js',
      ...
      request(
        ...
        function (err, _res, body) {
          ...
          text +=
            ...
            '\n' +
            fs.readFileSync(
              path.join(__dirname, 'libs', req.params.name + '.js')
            );
          res
            .set(
              Constants.HTTP_HEADER_CONTENT_TYPE,
              Constants.HTTP_CONTENT_TYPE_JS
            )
            .send(text.replace(/__OVEHOST__/g, Utils.getOVEHost()));
        }
      );
    }
  );
...
app.use(
  '/libs/distributed',
  express.static(path.join(__dirname, 'libs', 'distributed'))
);
